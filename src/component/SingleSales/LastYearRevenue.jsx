...
const LastYearRevenue = ({ years }) => {
  const { data, isLoading } = useChartData(
    `${process.env.REACT_APP_BASE_API_KEY}/v1/farms/comparison/revenue?period=monthly&year=${years}`,
    'LastYearRevenue'
  );

  return (
    <div className="flex flex-col w-[485px] min-h-[543px] bg-white rounded-[10px] pt-[18px] pl-[25px] pr-[14px] pb-[21px] gap-[15px] font-sans">
      <div className="text-lg font-bold">전년 대비 월별 매출 현황</div>
      {!isLoading && (
            ...
            <tbody className="bg-white">
              {data?.data.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 whitespace-nowrap border border-[#B9B9B9]">
                    {item.last_year_revenue?.toLocaleString('ko-KR')}
                  </td>
                  <td className="px-4 py-2 whitespace-nowrap border border-[#B9B9B9]">
                    {item.this_year_revenue?.toLocaleString('ko-KR')}
                  </td>
